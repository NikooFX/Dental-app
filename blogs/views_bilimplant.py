...
def blogs(request):
    ...
    if request.GET.get('blog', False):
        ...
        return render(request, 'blogs/bilimplant/blog.html', {'b': b})

    else:

        bg = Blogs.objects.filter(Q(company='all') | Q(company='bilimplant'))
        ...
        return render(request, 'blogs/bilimplant/blogs.html', {'bg':bg, 'pg': pg, 'pageminus': pageminus, 'pageplus': pageplus, 'page_number': page_number})