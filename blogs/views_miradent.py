...
def blogs(request):
    ...
    if request.GET.get('blog', False):
        ...
        return render(request, 'blogs/miradent/blog.html', {'b': b})

    else:

        bg = Blogs.objects.filter(Q(company='all') | Q(company='miradent'))
        ...
        return render(request, 'blogs/miradent/blogs.html', {'bg':bg, 'pg': pg, 'pageminus': pageminus, 'pageplus': pageplus, 'page_number': page_number})